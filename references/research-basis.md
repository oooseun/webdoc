...
- User feedback gathered in the website should be persisted to local agent-readable state. The NSFW triage tool is the local reference pattern: browser actions POST to a localhost API, the server validates and writes durable DB/file state, and the agent can inspect that state directly.
